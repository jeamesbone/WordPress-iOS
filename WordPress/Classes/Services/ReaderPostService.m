...
- (void)toggleSavedForPost:(ReaderPost *)post success:(void (^)())success failure:(void (^)(NSError *error))failure
{
    ...
    BOOL isOnlySavedPost = (readerPost.topics.count == 1 && [ReaderHelpers containsSavedPostsTopic:readerPost.topics]);
    BOOL canDelete = (readerPost.topics.count == 0 || isOnlySavedPost);
    if (!readerPost.isSaved && canDelete) {
